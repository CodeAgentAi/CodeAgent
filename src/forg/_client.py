...
from codeagentai.errors import BadRequest, NotFound, Unauthenticated


class Client:
    @classmethod
    def _get_http_headers(cls) -> Dict[str, str]:
        return {"Authorization": f"Bearer {codeagent.api_key}"}
    ...
    def request_action(
        ...
        url = f"https://{codeagent.api_base}/v1/{action_path}"
    ...
    async def listen_to_execution(cls, address: str) -> Union[str, bytes]:
        ...
        url = f"wss://{codeagent.api_base}/ws/v1/execution/{address}/"
