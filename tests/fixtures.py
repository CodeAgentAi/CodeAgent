...
def prepare() -> Dict:
    ...
    agent_builder_job = codeagent.AgentBuilderJob.list(agent_id=agent.id).results[0]
    build_id = None
    # Wait for the build to finish and get its id
    while True:
        time.sleep(3)
        builds = codeagent.Build.list(agent_builder_job_id=agent_builder_job.id).results
        if len(builds) == 1:
            build = builds[0]
...
def _delete_all_agents():
    for agent in _get_every_page_paginated_resource(codeagent.Agent):
